...
\item[\textbf{Task 1.2.}]
    ...
    \begin{enumerate}
        \item[1)]
            ...
            To calculate the homogeneous transformation from A to C, or short $^{A}T_{C}$, we need to right multiply $^{A}T_{B}$ and $^{B}T_{C}$. Intuitively we transform all points in A to B and then to C. That's why we need to right multiply the matrices. So the transformation $^{A}T_{C}$ is unambiguously defined.
